...
import { TOKEN_LOGOS } from "../../app/wallet/components/TokenPairsList";
...
export const TokenPairCard: React.FC<TokenPairCardProps> = ({
  ...
  const [isBalanceHidden, setIsBalanceHidden] = React.useState<boolean>(!pair.privateTokenBalance);
      ...
      <div className="flex flex-col gap-4 p-4 hover:bg-gray-50 rounded-xl transition-colors border border-gray-200 flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-soda-blue-900 flex items-center justify-center flex-shrink-0 overflow-hidden">
            {TOKEN_LOGOS[pair.clearAddress.toLowerCase()] ? (
              <img
                src={TOKEN_LOGOS[pair.clearAddress.toLowerCase()]}
                alt={pair.data.clearTokenSymbol ?? "token"}
                className="w-full h-full object-contain"
              />
            ) : (
              <span className="text-white font-bold text-sm">{pair.data.clearTokenSymbol?.charAt(0) ?? "T"}</span>
            )}
          </div>
          <div>
            <div className="font-semibold text-gray-900">{pair.data.clearTokenName ?? "Loading..."}</div>
            <div className="text-sm text-gray-500">{pair.data.clearTokenSymbol ?? "Loading..."}</div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="text-sm text-gray-500">
            {pair.clearTokenBalance && pair.data.clearTokenDecimals !== null && !isNaN(Number(pair.clearTokenBalance))
              ? ethers.formatUnits(pair.clearTokenBalance, pair.data.clearTokenDecimals)
              : "0.0"}{" "}
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
          <ActionButton
            ...
            isActive={
              bottomPanelType === "transfer-clear" && activeRowIndex === index && activeNetworkId === pair.chainId
            }
            onClick={() => {
              if (pair.chainId !== chainId) {
          ...
          <ActionButton
            ...
            title="Shield"
            disabled={!pair.clearTokenBalance || pair.clearTokenBalance === "0"}
            isActive={bottomPanelType === "shield" && activeRowIndex === index && activeNetworkId === pair.chainId}
            onClick={() => {
              if (pair.chainId !== chainId) {
          ...
          <ActionButton
            action="mint"
            title="Mint"
            isActive={bottomPanelType === "mint" && activeRowIndex === index && activeNetworkId === pair.chainId}
            onClick={() => {
              if (pair.chainId !== chainId) {
      ...
      <div className="flex flex-col gap-4 p-4 hover:bg-gray-50 rounded-xl transition-colors border border-gray-200 flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-10 h-10 rounded-full bg-gray-600 flex items-center justify-center flex-shrink-0">
            {TOKEN_LOGOS[pair.clearAddress.toLowerCase()] ? (
              <img
                src={TOKEN_LOGOS[pair.clearAddress.toLowerCase()]}
                alt={pair.data.privateTokenSymbol ?? "token"}
                className="w-full h-full object-contain"
              />
            ) : (
              <span className="text-white font-bold text-sm">{pair.data.privateTokenSymbol?.charAt(0) ?? "P"}</span>
            )}
            {/* small shield badge */}
          ...
          <div>
            <div className="font-semibold text-gray-900">{pair.data.privateTokenName ?? "Loading..."}</div>
            <div className="text-sm text-gray-500">{pair.data.privateTokenSymbol ?? "Loading..."}</div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
          <ActionButton
            ...
            title="Send"
            isActive={
              bottomPanelType === "transfer-private" && activeRowIndex === index && activeNetworkId === pair.chainId
            }
            onClick={() => {
              if (pair.chainId !== chainId) {
          ...
          <ActionButton
            action="unshield"
            title="Unshield"
            isActive={bottomPanelType === "unshield" && activeRowIndex === index && activeNetworkId === pair.chainId}
            onClick={() => {
              if (pair.chainId !== chainId) {
